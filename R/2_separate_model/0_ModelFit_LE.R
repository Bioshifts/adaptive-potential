...
mydatatogo$Param <- relevel(mydatatogo$Param, ref = "LE") 

################################################################################
###############Trailing edge model analysis#####################################
###############Fit a single model with GD*climate velocity######################
################################################################################
mydatatogo=subset(mydatatogo,Param=="LE")
...
subset(res_ok,model=="LE" & pv.sup0<0.05)

# #significant negative effect
subset(res_ok,model=="LE" & pv.inf0<0.05)

# #non-significant negative effect
subset(res_ok,model=="LE" & pv.inf0>=0.05 & pv.sup0>=0.05)

### Test climate velocity value for which GD is significant
#### TE model
nB=r2a$nB
c1=read.csv2(here(dir.out,"coeff_LE.csv"),
             sep=";",dec=".",h=T)
dsel=mydatatogo
...
ex=pblapply(nB,function(i){
    ...
    tmp=data.frame(vel_abs=v1,GDeff_LE=eff)
    tmp$nB=i
    return(tmp)
}, cl = my.cluster2)
...
write.table(resLE,
            here(dir.out,"summary_GDeff_LE.csv"),
            sep=";",dec=".",row=F)
...
ex=pblapply(nB,function(i){
    ...
    tmp=data.frame(GD=v1,VAeff_LE=eff)
    tmp$nB=i
    return(tmp)
}, cl = my.cluster2)
...
write.table(resLE,
            here(dir.out,"summary_VAeff_LE.csv"),
            sep=";",dec=".",row=F)
